...
static my_bool free_transaction_handlerton(THD *thd, plugin_ref plugin,
                                           void *unused)
{
  handlerton *hton= plugin_hton(plugin);
  if (hton->free_transaction)
    hton->free_transaction(hton, thd);
  return FALSE;
}


void ha_free_transactions(THD *thd)
{
  plugin_foreach(thd, free_transaction_handlerton, MYSQL_STORAGE_ENGINE_PLUGIN,
                 0);
}
